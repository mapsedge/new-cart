...
$mode   = DB::val("SELECT `value` FROM `" . DB_PREFIX . "settings` WHERE `key` = 'stripe_mode'") ?: 'test';
$secret = $mode === 'live'
	? (DB::val("SELECT `value` FROM `" . DB_PREFIX . "settings` WHERE `key` = 'stripe_webhook_secret'") ?: '')
	: (DB::val("SELECT `value` FROM `" . DB_PREFIX . "settings` WHERE `key` = 'stripe_test_webhook_secret'") ?: '');
